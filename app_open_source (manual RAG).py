...
def get_cache_key(raw_text):
    cache_basis = (
        ...
        f"embedding=all-MiniLM-L6-v2|top_k={TOP_K}|manual_rag=final|minimal_fallback=yes"
    )
...
def retrieve_documents(vectorstore, question, top_k=TOP_K):
    docs = vectorstore.similarity_search(question, k=top_k)
    print("STEP R1: retrieved documents", flush=True)
...
def answer_document_question(llm, vectorstore, user_question):
    ...
    print("STEP R2: generating answer from manual RAG prompt", flush=True)
    answer = llm(final_prompt).strip()
...
def handle_userinput(user_question):
    ...
    if st.session_state.vectorstore is None or st.session_state.llm is None:
        ...
        print("STEP E2 STOP: vectorstore or llm is None", flush=True)
        return
...
def main():
            ...
            if not pdf_docs:
                ...
                with st.spinner("Processing"):
                    try:
                        ...
                        print("STEP P6 DONE: processing complete", flush=True)
    ...
    st.caption("Manual RAG with light answer cleaning and a minimal numeric fallback.")
